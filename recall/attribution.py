...
from .runtime import run_task, is_harmful, K
...
def _harmful_without(store, agent, task, removed):
    return is_harmful(run_task(store, agent, task, exclude=set(removed), record=False), task)


def diagnose(store, agent, task, use_provenance_prior: bool = True) -> Diagnosis:
    start = agent.calls
    base = run_task(store, agent, task, record=False)
    d = Diagnosis(task["id"], is_harmful(base, task))
    if not d.harmful:
    ...
    # 1) Leave-one-out.
    for m in retrieved:
        d.loo[m.id] = 0 if _harmful_without(store, agent, task, [m.id]) else 1
    ...
    for m in order:
        removed.append(m.id)
        if not _harmful_without(store, agent, task, removed):
            break
    else:
    ...
    for mid in list(reversed(removed)):
        ...
        if not _harmful_without(store, agent, task, trial):
            removed = trial
